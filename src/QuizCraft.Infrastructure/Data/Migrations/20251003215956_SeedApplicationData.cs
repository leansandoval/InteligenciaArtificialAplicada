...
namespace QuizCraft.Infrastructure.Data.Migrations
{
    ...
    public partial class SeedApplicationData : Migration
    {
        ...
        {
            // Insertar Materias (solo si no existen - usando MERGE para idempotencia)
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    ) AS source (Id, Nombre, Descripcion, Color, EstaActivo, FechaCreacion, FechaModificacion, Icono, UsuarioId)
                    ON target.Id = source.Id
                    WHEN NOT MATCHED THEN
                        INSERT (Id, Nombre, Descripcion, Color, EstaActivo, FechaCreacion, FechaModificacion, Icono, UsuarioId)
                        VALUES (source.Id, source.Nombre, source.Descripcion, source.Color, source.EstaActivo, source.FechaCreacion, source.FechaModificacion, source.Icono, source.UsuarioId);
                    
                    SET IDENTITY_INSERT Materias OFF
                END
            ");

            // Insertar Flashcards (solo si no existen - usando MERGE para idempotencia)
            migrationBuilder.Sql(@"
                SET IDENTITY_INSERT Flashcards ON
                
                MERGE INTO Flashcards AS target
                USING (VALUES 
                    -- Matemáticas
                ...
                ) AS source (Id, Pregunta, Respuesta, Dificultad, EstaActivo, FechaCreacion, MateriaId, Pista, VecesVista, VecesCorrecta, VecesIncorrecta, IntervaloRepeticion, FactorFacilidad)
                ON target.Id = source.Id
                WHEN NOT MATCHED THEN
                    INSERT (Id, Pregunta, Respuesta, Dificultad, EstaActivo, FechaCreacion, MateriaId, Pista, VecesVista, VecesCorrecta, VecesIncorrecta, IntervaloRepeticion, FactorFacilidad)
                    VALUES (source.Id, source.Pregunta, source.Respuesta, source.Dificultad, source.EstaActivo, source.FechaCreacion, source.MateriaId, source.Pista, source.VecesVista, source.VecesCorrecta, source.VecesIncorrecta, source.IntervaloRepeticion, source.FactorFacilidad);
                
                SET IDENTITY_INSERT Flashcards OFF
            ");

            // Insertar Quizzes (usando MERGE para idempotencia)
            migrationBuilder.Sql(@"
                DECLARE @AdminUserId NVARCHAR(450)
                SELECT @AdminUserId = Id FROM AspNetUsers WHERE Email = 'admin@quizcraft.com'
                ...
                BEGIN
                    ...
                    ) AS source (Id, Titulo, Descripcion, EstaActivo, FechaCreacion, MateriaId, CreadorId, TiempoLimite, NivelDificultad, EsPublico, NumeroPreguntas, EsActivo, TiempoPorPregunta, MostrarRespuestasInmediato, PermitirReintento)
                    ON target.Id = source.Id
                    WHEN NOT MATCHED THEN
                        INSERT (Id, Titulo, Descripcion, EstaActivo, FechaCreacion, MateriaId, CreadorId, TiempoLimite, NivelDificultad, EsPublico, NumeroPreguntas, EsActivo, TiempoPorPregunta, MostrarRespuestasInmediato, PermitirReintento)
                        VALUES (source.Id, source.Titulo, source.Descripcion, source.EstaActivo, source.FechaCreacion, source.MateriaId, source.CreadorId, source.TiempoLimite, source.NivelDificultad, source.EsPublico, source.NumeroPreguntas, source.EsActivo, source.TiempoPorPregunta, source.MostrarRespuestasInmediato, source.PermitirReintento);
                    
                    SET IDENTITY_INSERT Quizzes OFF
                END
            ");

            // Insertar Preguntas de Quiz (usando MERGE para idempotencia)
            migrationBuilder.Sql(@"
                SET IDENTITY_INSERT PreguntasQuiz ON
                
                MERGE INTO PreguntasQuiz AS target
                USING (VALUES 
                -- Quiz Matemáticas
                ...
                ) AS source (Id, TextoPregunta, TipoPregunta, OpcionA, OpcionB, OpcionC, OpcionD, RespuestaCorrecta, Explicacion, Puntos, Orden, EstaActivo, FechaCreacion, QuizId)
                ON target.Id = source.Id
                WHEN NOT MATCHED THEN
                    INSERT (Id, TextoPregunta, TipoPregunta, OpcionA, OpcionB, OpcionC, OpcionD, RespuestaCorrecta, Explicacion, Puntos, Orden, EstaActivo, FechaCreacion, QuizId)
                    VALUES (source.Id, source.TextoPregunta, source.TipoPregunta, source.OpcionA, source.OpcionB, source.OpcionC, source.OpcionD, source.RespuestaCorrecta, source.Explicacion, source.Puntos, source.Orden, source.EstaActivo, source.FechaCreacion, source.QuizId);
